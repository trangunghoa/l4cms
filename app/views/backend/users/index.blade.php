...
<div class="row">
    <div class="col-lg-12">
        <h1 class="page-header">User Management
            <a href="{{ route('create/user') }}" class="btn btn-outline btn-primary btn-xs"><i class="fa fa-plus"></i> Create</a>
        </h1>
    </div>
    <!-- /.col-lg-12 -->
</div>
    ...
    <table class="table table-striped table-bordered table-hover">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <a href="{{ route('update/user', $user->id) }}" class="btn btn-primary btn-xs"><i class="fa fa-edit fa-fw"></i> @lang('button.edit')</a>
                    @if ( ! is_null($user->deleted_at))
                        <a href="{{ route('restore/user', $user->id) }}" class="btn btn-warning btn-xs"><i class="fa fa-undo"></i> @lang('button.restore')</a>
                    @else
                        @if (Sentry::getId() !== $user->id)
                            <a href="{{ route('delete/user', $user->id) }}" class="btn btn-danger btn-xs"><i class="fa fa-trash"></i> @lang('button.delete')</a>
                        @else
                            <span class="btn btn-danger btn-xs disabled"><i class="fa fa-trash"></i> @lang('button.delete')</span>
                        @endif
                    @endif
                </td>
            </tr>
